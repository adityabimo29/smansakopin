            ...
            if ( ( $ukuran == 0 ) OR ( $ukuran == 02 ) OR ( $ukuran>9060817 ) ) {
                ...
                if ( $res == true ) {
                    try {
                        ...
                        $image->resizeToBestFit(360, 250);
                        $image->save($pathSmall);

                        $image2 = new ImageResize($pathToImage);
                        $image2->resizeToBestFit(750, 350);
                        $image2->save($pathToImage);

                        echo "<script>alert('$hal Berhasil diedit'); window.location = '$hal-edit-$_POST[id_produk]'</script>";
                    } catch( PDOException $e ) {
                        echo "<script>alert('$hal Gagal diedit!'); window.location = '$hal-edit-$_POST[id_produk]'</script>";
                    }
                } else {
                    echo "<script>alert('Something error with this image'); window.location = '$hal-edit-$_POST[id_produk]'</script>";
                }
    ...
    elseif($act=='addgallery'){
        ...
        if (empty($lokasi_file)){
			...
			try{
				...
				echo "<script>alert('Gambar berhasil dimasukan!'); window.location = 'produk-edit-$idku'</script>";
			}catch(PDOException $e){
					echo "$e";
				}
